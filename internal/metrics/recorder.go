...
func (r *AsyncRecorder) toStoredEvent(event Event) StoredEvent {
	...
	roomLabel := strings.TrimSpace(r.roomAliases[event.RawRoomID])
	if roomLabel == "" {
	...
	return StoredEvent{
		...
		RoomIDHash:       r.hash(event.RawRoomID),
		TenantIDHash:     r.hash(event.RawTenantID),
		RoomScopeHash:    r.hash(rawScopeRoomID),
		RoomLabel:        roomLabel,
		RoomNameSnapshot: strings.TrimSpace(event.RoomName),
		UserIDHash:       r.hash(event.RawUserID),
...
func (r *AsyncRecorder) hash(value string) string {
