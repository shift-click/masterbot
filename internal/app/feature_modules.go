		...
		basicFeatureModule{
			...
			build: func(ctx *featureBuildContext) (featureRuntime, error) {
				return featureRuntime{
					...
					// AcceptACLDenied: true keeps the probe meaningful when the
					// smoke runner uses adminSmokeSyntheticChatID — the synthetic
					// id is intentionally not on the ACL allowlist, so router
					// will respond with bot.DeniedIntentMessage. The probe still
					// proves routing + ACL evaluation are alive end-to-end.
					smokeProbes: []admin.CommandSmokeProbe{
						{
							ID:              "help",
							Message:         "도움",
		...
		basicFeatureModule{
			...
			build: func(ctx *featureBuildContext) (featureRuntime, error) {
				return featureRuntime{
					...
					// calc is implemented as an ACL-exempt fallback, so it
					// reaches a normal success path even from the synthetic
					// chat id. AcceptACLDenied: true is set for consistency
					// with other probes; it has no effect when the reply is
...
// adminSmokeSyntheticChatID is a URI-shaped synthetic chat id used by the
// admin command smoke runner when no explicit smoke chat id is configured.
// It deliberately uses a scheme prefix that KakaoTalk never assigns so the
// derived `room_id_hash` cannot collide with any operational room hash and
// pollute production metrics.
const adminSmokeSyntheticChatID = "admin-smoke://synthetic"
...
func (r commandSmokeRunner) Run(ctx context.Context, probes []admin.CommandSmokeProbe) ([]admin.CommandSmokeResult, error) {
	...
	for _, probe := range probes {
		...
		ok, aclDenied := commandSmokeMatchesExpectation(replies, probe)
		result.OK = ok
		result.ACLDenied = aclDenied
...
func resolveSmokeRoomChatID(cfg config.Config) string {
	if room := strings.TrimSpace(cfg.Admin.SmokeRoomChatID); room != "" {
		return room
	}
	// When no explicit smoke chat id is configured we MUST NOT borrow the
	// bootstrap admin room or the first access-list room, because doing so
	// poisons that operational room's `room_id_hash` with synthetic smoke
	// events (access_denied/command_dispatched/activation/...). Returning a
	// schemed synthetic id keeps smoke metrics in their own hash bucket.
	return adminSmokeSyntheticChatID
}

func commandSmokeMatchesExpectation(replies []bot.Reply, probe admin.CommandSmokeProbe) (matched bool, aclDenied bool) {
	if len(replies) == 0 {
		return false, false
	}
	if probe.AcceptACLDenied && len(replies) == 1 &&
		replies[0].Type == transport.ReplyTypeText &&
		strings.TrimSpace(replies[0].Text) == bot.DeniedIntentMessage {
		return true, true
	}
