...
.PHONY: help build build-arm64 run dev test lint tidy verify verify-public-cicd e2e install-hooks package-release export-public publish-public deploy-remote deploy-validate deploy-history full-deploy pull-secrets remote-bootstrap remote-up-redroid remote-up-jucobot remote-rollback sonar-main quality-gate tf-init tf-plan tf-apply tf-destroy tf-output deploy-aws aws-up-jucobot
...
verify-public-cicd: ## public CI/CD 로컬 검증
	./scripts/verify/public-cicd-local.sh
...
export-public: ## 공개용 스냅샷 생성 및 검증
	./scripts/export-public.sh

publish-public: ## 공개 레포에 새 히스토리로 push
