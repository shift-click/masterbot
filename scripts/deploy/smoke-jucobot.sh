...
if [ "$started" -ne 1 ]; then
  ...
  docker compose --env-file "$COMPOSE_ENV_FILE" -f "$COMPOSE_FILE" ps >&2 || true
  exit 1
fi
...
if [ "${JUCOBOT_ALERTD_ENABLED:-false}" = "true" ]; then
  ...
  if [ "$alertd_running" != "true" ]; then
    ...
    docker compose --env-file "$COMPOSE_ENV_FILE" -f "$COMPOSE_FILE" --profile alerting ps >&2 || true
    exit 1
  fi
fi
...
if [ -n "${JUCOBOT_CHART_RENDERER_URL:-}" ]; then
  ...
  if [ "$chart_ready" -ne 1 ]; then
    ...
    docker compose --env-file "$COMPOSE_ENV_FILE" -f "$COMPOSE_FILE" --profile chart ps >&2 || true
